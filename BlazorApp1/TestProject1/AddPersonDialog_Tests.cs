...
public class AddPersonDialog_Tests : TestBase
{
    ...
    {
        // Arrange
        var personToAdd = _fixture.Create<Person>();
        ...
        // Assert 
        A.CallTo(() => _fakeApiService.SaveAsync(
    ...
    public void GIVEN_add_person_dialog_WHEN_not_adding_first_name_THEN_a_call_to_client_error_handling_service_with_correct_message_should_have_happened()
    {
        ...
        A.CallTo(() => _fakeClientErrorHandlingService.HandleError("Validation Error", false)).MustHaveHappened();
        A.CallTo(() => _fakeApiService.SaveAsync(person)).MustNotHaveHappened();
        A.CallTo(() => _fakeDialogInstance.Close(A<DialogResult>.Ignored)).MustNotHaveHappened();
    }
